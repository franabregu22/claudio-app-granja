-- ============================================================================
-- TEST: import_financial_movements_reconciliation_v2 with migration 010
-- Purpose: Validate 4-class classification + idempotence + rollback
-- ============================================================================
...
-- ============================================================================
-- FIRST EXECUTION: 4 test rows (payment+, payment-, asset_mgmt, payout)
-- ============================================================================
...
SELECT import_financial_movements_reconciliation_v2(
  ...
  ARRAY[
    jsonb_build_object(
      'DATE', '2026-09-11T10:00:00.000-03:00',
      'SOURCE_ID', 'test_payment_plus_001',
      'DESCRIPTION', 'payment',
    ...
    jsonb_build_object(
      ...
      'SOURCE_ID', 'test_payment_minus_001',
      'DESCRIPTION', 'payment',
      'NET_CREDIT_AMOUNT', '0.00',
      'NET_DEBIT_AMOUNT', '40.00',
      'GROSS_AMOUNT', '40.00',
      'MP_FEE_AMOUNT', '0.00',
      'TAXES_AMOUNT', '0.00',
      'PAYMENT_METHOD', 'available_money'
    ...
    jsonb_build_object(
      ...
      'SOURCE_ID', 'test_asset_mgmt_001',
      'DESCRIPTION', 'asset_management',
      'NET_CREDIT_AMOUNT', '5.00',
      'NET_DEBIT_AMOUNT', '0.00',
      'GROSS_AMOUNT', '5.00',
      'MP_FEE_AMOUNT', '0.00',
      'TAXES_AMOUNT', '0.00',
    ...
    jsonb_build_object(
      'DATE', '2026-09-11T13:00:00.000-03:00',
      'SOURCE_ID', 'test_payout_001',
      'DESCRIPTION', 'payout',
      'NET_CREDIT_AMOUNT', '0.00',
      'NET_DEBIT_AMOUNT', '20.00',
      'GROSS_AMOUNT', '20.00',
      'MP_FEE_AMOUNT', '0.00',
      'TAXES_AMOUNT', '0.00',
      'PAYMENT_METHOD', 'available_money'
...
-- Validate classifications and impacts
SELECT
  mfm.id,
  mfm.movement_class,
...
WHERE mfm.account_id = 1054315166
  AND msr.source_external_id LIKE 'test_%'
ORDER BY mfm.id;
...
-- ============================================================================
-- VALIDATION: Compare baseline → after_first → after_second
-- ============================================================================
...
SELECT
  ...
  (SELECT COUNT(*) FROM mp_financial_movement WHERE account_id = 1054315166
    AND id > (SELECT MAX(id) - 10 FROM mp_financial_movement)) as recent_fm_count;
