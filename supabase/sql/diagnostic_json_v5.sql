-- ============================================================================
-- DIAGNOSTIC v5: Test INSERT en mp_source_record + mp_financial_movement + mp_movement_source_link
-- Con rollback seguro, sin ledger_entry
-- Reproduce LITERALMENTE la lógica real de import_financial_pipeline_rpc_v2
-- ============================================================================

CREATE OR REPLACE FUNCTION diagnostic_json_v5(p_input JSONB)
RETURNS JSONB AS $$
DECLARE
  v_records JSONB[];
  v_record JSONB;
...
BEGIN
      ...
      BEGIN
        ...
        IF v_source_record_id IS NOT NULL THEN
          ...
          IF v_existing_fm_id IS NULL THEN
            -- ================================================================
            -- SCENARIO C: Primer source → crear financial_movement + link
            -- Reproduce LITERALMENTE líneas 157-192 de import_financial_pipeline_rpc_v2
            -- ================================================================
            ...
            -- ================================================================
            -- Crear movement_source_link (líneas 188-192 de v2)
            -- ================================================================
            ...
            v_movement_links_new := v_movement_links_new + 1;

            -- NOTA: NO crear ledger_entry en v5
          END IF;
  ...
  WHEN SQLSTATE 'PZ001' THEN
    ...
    RETURN jsonb_build_object(
      ...
      'movement_links_new', v_movement_links_new
    );
