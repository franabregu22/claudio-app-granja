...
export function ListaMovimientos({ movimientos, onAnular }: ListaMovimientosProps) {
  ...
  return (
    <>
      {/* MOBILE VERSION */}
      <div className="bg-white rounded-lg border border-[#E4DCC8] overflow-x-auto md:hidden">
        <table className="w-full text-xs">
          <thead className="bg-amber-50 border-b border-[#D8CDB0]">
            <tr>
              <th className="px-2 py-2 text-left font-semibold text-[#2C2419]">Fecha Op.</th>
              <th className="px-2 py-2 text-left font-semibold text-[#2C2419]">Nombre</th>
              <th className="px-2 py-2 text-right font-semibold text-[#2C2419]">Monto</th>
              <th className="px-2 py-2 text-center font-semibold text-[#2C2419]">✓</th>
            </tr>
          </thead>
          <tbody>
            {paginatedMovimientos.map((m) => (
              ...
              >
                <td className="px-2 py-2 text-[#2C2419] font-medium text-xs">{m.fecha_operacion}</td>
                <td className="px-2 py-2">
                  <p className="font-semibold truncate text-xs text-[#2C2419]">
                    {m.concepto}
                  </p>
                </td>
                  ...
                  {m.tipo === 'ingreso' && m.forma_pago === 'efectivo' && m.movimiento_estado === 'confirmado' ? (
                    m.agregado_a_caja ? (
                      ...
                      <button
                        ...
                        className="flex items-center justify-center p-0.5 hover:bg-green-50 rounded transition disabled:opacity-50"
      ...
      {/* DESKTOP VERSION */}
      <div className="bg-white rounded-lg border border-[#E4DCC8] overflow-x-auto hidden md:block">
