-- ============================================================================
-- FINAL PRODUCTION IMPORT FOR JUNE 2026
-- Account: 1054315166
-- Execution: Manual in Supabase Studio → SQL Editor
-- ============================================================================

-- PART 1: Create/Replace the WRITE version of the reconciliation function
-- (This uses the same logic as preview but actually writes to DB)
...
BEGIN
  -- Get current ledger balance for June
  ...
  FOREACH v_row IN ARRAY p_input_rows LOOP
    ...
    -- Layer 1: Check if SR already exists (idempotent)
    ...
    IF v_is_raw_only THEN
      v_raw_only_rows := v_raw_only_rows + 1;
    ELSE
      -- Layer 3: Check for existing FM to reuse (Liberaciones)
...
-- ============================================================================
-- PART 2: EXECUTE IMPORT (Report first, then Liberaciones)
-- ============================================================================

-- First: Import Report (682 rows)
SELECT import_financial_movements_reconciliation_v2(
  1054315166,
  ARRAY[/* Report batch JSON array here - 682 rows */]::jsonb[],
  'report',
  '2026-06-01'::DATE,
  '2026-06-30'::DATE
) AS report_import_result;

-- Second: Import Liberaciones (744 rows)
SELECT import_financial_movements_reconciliation_v2(
  1054315166,
  ARRAY[/* Liberaciones batch JSON array here - 744 rows */]::jsonb[],
  'liberaciones',
  '2026-06-01'::DATE,
  '2026-06-30'::DATE
...
-- ============================================================================
-- PART 3: POST-IMPORT VERIFICATION (READ-ONLY)
-- ============================================================================

SELECT
  'June 2026 Post-Import Verification' AS verification_type,

  12343127.31::NUMERIC AS ledger_baseline_before_import,

  (SELECT COALESCE(SUM(le.balance_impact), 0) FROM ledger_entry le
   INNER JOIN mp_financial_movement mfm ON le.financial_movement_id = mfm.id
   WHERE mfm.account_id = 1054315166
     AND DATE(le.occurred_at AT TIME ZONE 'America/Argentina/Buenos Aires') BETWEEN '2026-06-01' AND '2026-06-30') AS ledger_after,

  (SELECT COALESCE(SUM(le.balance_impact), 0) FROM ledger_entry le
   INNER JOIN mp_financial_movement mfm ON le.financial_movement_id = mfm.id
   WHERE mfm.account_id = 1054315166
     AND DATE(le.occurred_at AT TIME ZONE 'America/Argentina/Buenos Aires') BETWEEN '2026-06-01' AND '2026-06-30') - 12343127.31 AS import_delta,

  (SELECT COUNT(*) FROM (
    SELECT le.financial_movement_id, COUNT(*) AS cnt
    FROM ledger_entry le
    INNER JOIN mp_financial_movement mfm ON le.financial_movement_id = mfm.id
    WHERE mfm.account_id = 1054315166
      AND DATE(le.occurred_at AT TIME ZONE 'America/Argentina/Buenos Aires') BETWEEN '2026-06-01' AND '2026-06-30'
    GROUP BY le.financial_movement_id
    HAVING COUNT(*) > 1
  ) dupes) AS duplicate_le_per_fm,

  (SELECT COUNT(DISTINCT mfm.id) FROM mp_financial_movement mfm
   INNER JOIN ledger_entry le ON mfm.id = le.financial_movement_id
   WHERE mfm.account_id = 1054315166
     AND mfm.needs_review = TRUE
     AND DATE(le.occurred_at AT TIME ZONE 'America/Argentina/Buenos Aires') BETWEEN '2026-06-01' AND '2026-06-30') AS needs_review_count,

  (SELECT COUNT(DISTINCT mfm.id) FROM mp_financial_movement mfm
   INNER JOIN ledger_entry le ON mfm.id = le.financial_movement_id
   WHERE mfm.account_id = 1054315166
     AND ABS(le.balance_impact - 1629.44) < 0.01
     AND DATE(le.occurred_at AT TIME ZONE 'America/Argentina/Buenos Aires') BETWEEN '2026-06-01' AND '2026-06-30') AS fm_with_1629_44,

  (SELECT COUNT(DISTINCT mfm.id) FROM mp_financial_movement mfm
   INNER JOIN ledger_entry le ON mfm.id = le.financial_movement_id
   WHERE mfm.account_id = 1054315166
     AND ABS(le.balance_impact - 463.84) < 0.01
     AND DATE(le.occurred_at AT TIME ZONE 'America/Argentina/Buenos Aires') BETWEEN '2026-06-01' AND '2026-06-30') AS fm_with_463_84,
  ...
  (SELECT COUNT(DISTINCT mfm.id) FROM mp_financial_movement mfm
   ...
   WHERE mfm.account_id = 1054315166
     ...
     AND DATE(le.occurred_at AT TIME ZONE 'America/Argentina/Buenos Aires') BETWEEN '2026-06-01' AND '2026-06-30') AS payout_fm_count;
