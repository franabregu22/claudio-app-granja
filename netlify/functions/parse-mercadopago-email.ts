...
const handler: Handler = async (event) => {
  ...
  try {
    ...
    // SendGrid sends form data, extract CSV from attachments
    let csvContent = "";

    if (data.attachments) {
      const attachments = JSON.parse(data.attachments);
      for (const att of attachments) {
        if (att.filename && att.filename.endsWith(".csv")) {
          // attachment content is base64 encoded
          csvContent = Buffer.from(att.content, "base64").toString("utf-8");
          break;
        }
      }
    }
    ...
    if (!csvContent) {
      return {
        statusCode: 400,
        body: JSON.stringify({ error: "No CSV found in email" }),
        headers
      };
    }
    ...
    if (lines.length < 2) {
      return {
        statusCode: 400,
        body: JSON.stringify({ error: "CSV empty or invalid" }),
        headers
      };
    }
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      const sourceId = record["Id de Liquidación"] || record["Id"] || `${Date.now()}-${i}`;
      const dateStr = record["Fecha"] || new Date().toISOString();
      const creditStr = record["Crédito Neto"] || "0";
      const debitStr = record["Débito Neto"] || "0";

      const creditAmount = parseFloat(creditStr.replace(",", ".")) || 0;
      const debitAmount = parseFloat(debitStr.replace(",", ".")) || 0;
      ...
      if (movementType.toUpperCase().includes("PAYMENT") || movementType.toUpperCase().includes("PAGO")) {
        ...
        // Financial movements (comisiones, impuestos, rendimientos, etc)
        movements.push({
          id: sourceId,
