-- ============================================================================
-- VALIDATE FEB 2026 IMPORT RESULTS — SPLIT INTO 5 INDEPENDENT QUERIES
-- File: 026_validate_feb_import_SPLIT.sql
-- Status: READ-ONLY
-- Execute EACH query separately in Supabase SQL Editor
-- ============================================================================

-- ============================================================================
-- QUERY 1: FEB LEDGER SUMMARY
-- ============================================================================
-- Execute this first
...
-- ============================================================================
-- QUERY 2: FEB LIBERACIONES DESCRIPTION SUMMARY
-- ============================================================================
-- Execute this second
...
SELECT
  ...
  (SELECT COUNT(DISTINCT msr.id) FROM mp_source_record msr
   ...
   WHERE msr.source_type = 'liberaciones'
     ...
     AND mfm.account_id = 1054315166
     AND DATE(mfm.transaction_date AT TIME ZONE 'America/Argentina/Buenos_Aires') BETWEEN '2026-02-01'::date AND '2026-02-28'::date) as source_rows,
...
SELECT
  (SELECT COUNT(DISTINCT msr.id) FROM mp_source_record msr
   ...
   WHERE msr.source_type = 'liberaciones'
     AND msr.raw_data->>'DESCRIPTION' = 'payout'
     AND mfm.account_id = 1054315166
     AND DATE(mfm.transaction_date AT TIME ZONE 'America/Argentina/Buenos_Aires') BETWEEN '2026-02-01'::date AND '2026-02-28'::date) as payout_source_records,
