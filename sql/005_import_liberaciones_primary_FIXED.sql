-- RPC: import_liberaciones_primary FIXED
-- Atomic batch import of Liberaciones.csv records
-- SCHEMA-COMPATIBLE: respeta estrutura real de tablas
...
BEGIN
  ...
  FOR v_idx IN 1 .. array_length(p_input, 1) LOOP
    ...
    -- NOTA: NO insertar account_id (no existe en mp_source_record)
    ...
    -- Buscar SR anterior del mismo SOURCE_ID
    ...
    IF v_prev_sr_id IS NOT NULL THEN
      ...
      IF v_prev_fm_id IS NOT NULL THEN
        -- Comparar economic hashes
        SELECT economic_hash INTO v_prev_economic_hash
        ...
        v_current_economic_hash := calc_economic_hash(
          ...
          NULL,  -- movement_class (se asigna luego)
          v_transaction_date,  -- transaction_date
          NULL,  -- settlement_date
          v_payment_method,  -- payment_method
          v_payment_method_type,  -- payment_detail (usando payment_method_type)
          NULL  -- tax_detail (no disponible en Liberaciones)
        );

        IF v_prev_economic_hash IS NOT NULL AND v_prev_economic_hash != v_current_economic_hash THEN
    ...
    ELSE
      ...
      -- Calcular economic_hash para el nuevo FM
      v_current_economic_hash := calc_economic_hash(
        v_gross_amount,
        v_balance_impact,
        v_tax_amount,
      ...
      INSERT INTO mp_financial_movement (
        ...
        needs_review, economic_hash, created_at
      ) VALUES (
        p_account_id, v_movement_class, v_gross_amount, v_balance_impact,
        v_tax_amount, v_payment_method, v_payment_method_type, v_transaction_date,
        v_economic_changed, v_current_economic_hash, NOW()
      )
      ...
      INSERT INTO ledger_entry (
        ...
        category, description, occurred_at, created_at
      ) VALUES (
    ...
    INSERT INTO mp_movement_source_link (
      financial_movement_id, source_record_id,
      is_primary,
      linked_at
    ) VALUES (
      v_financial_movement_id, v_source_record_id,
      (v_fm_created > 0),  -- is_primary=TRUE si FM fue creado por este SR
      NOW()
    )
...
COMMENT ON FUNCTION import_liberaciones_primary(BIGINT, JSONB[]) IS
  'Atomic batch import of Liberaciones records. SCHEMA-COMPATIBLE: no account_id in SR, settlement_amount in FM, LE 1:1 per FM. Supports: 1) same-source economic change detection, 2) report correlation for payments/assets, 3) payout creation with idempotence, 4) checkpoint-based partial rollback.';
