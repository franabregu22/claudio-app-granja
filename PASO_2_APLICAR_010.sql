-- PASO 2: APLICAR MIGRATION 010 - COPIAR TODO Y PEGAR EN SUPABASE SQL EDITOR
-- ============================================================================
...
BEGIN
  ...
  FOREACH v_row IN ARRAY p_input_rows LOOP
    ...
    v_economic_row_fp := compute_economic_row_fp(p_account_id, p_source_type, v_source_id, v_signed_impact, (v_row->>'DATE')::TIMESTAMP, v_description);
    ...
    IF p_source_type = 'report' AND v_explicit_payload_hash IS NOT NULL THEN
      ...
      WHERE msr.source_type = p_source_type AND msr.payload_hash = v_explicit_payload_hash AND msr.account_id = p_account_id LIMIT 1;
    ELSE
      SELECT msr.id, msr.account_id, msr.source_external_id INTO v_existing_sr
      FROM mp_source_record msr
      WHERE msr.source_type = p_source_type AND md5(msr.raw_data::text) = v_payload_hash AND msr.account_id = p_account_id LIMIT 1;
    END IF;

    IF FOUND THEN
      v_existing_raw_exact := v_existing_raw_exact + 1;
      IF v_is_raw_only THEN v_raw_only_rows := v_raw_only_rows + 1; END IF;
    ...
    INSERT INTO mp_source_record (source_type, source_external_id, raw_data, account_id, economic_row_fp, cross_source_fp, payload_hash, observed_at)
    VALUES (p_source_type, v_source_id, v_row, p_account_id, v_economic_row_fp, v_cross_source_fp, v_payload_hash, NOW())
    ...
    IF NOT v_is_raw_only THEN
      ...
      ELSE
        SELECT fm_id, source_count, settlement_amount INTO v_existing_fm
        FROM get_fm_candidates_by_cross_fp(p_account_id, v_cross_source_fp);
        IF FOUND AND ABS(v_existing_fm.settlement_amount - v_signed_impact) > 0.01 THEN v_existing_fm := NULL; END IF;
        IF v_existing_fm IS NOT NULL THEN
          SELECT COUNT(*) INTO v_idx FROM get_fm_candidates_by_cross_fp(p_account_id, v_cross_source_fp);
          IF v_idx > 1 THEN v_ambiguous_rows := v_ambiguous_rows + 1; v_existing_fm := NULL; END IF;
        END IF;
      END IF;

      IF FOUND AND v_existing_fm IS NOT NULL THEN
        INSERT INTO mp_movement_source_link (financial_movement_id, source_record_id, is_primary) VALUES (v_existing_fm.fm_id, v_new_sr_id, FALSE)
        RETURNING id INTO v_new_link_id;
        v_link_ids := array_append(v_link_ids, v_new_link_id);
      ELSE
        INSERT INTO mp_financial_movement (account_id, movement_class, settlement_amount, transaction_date, needs_review)
        VALUES (p_account_id, v_movement_class_final, v_signed_impact, (v_row->>'DATE')::TIMESTAMP, FALSE)
        ...
        INSERT INTO ledger_entry (financial_movement_id, account_id, balance_impact, category, occurred_at)
        VALUES (v_new_fm_id, p_account_id, v_signed_impact, v_ledger_category_final, (v_row->>'DATE')::TIMESTAMP)
        RETURNING id INTO v_new_le_id;
        v_new_ledger_entries := v_new_ledger_entries + 1;
        v_le_ids := array_append(v_le_ids, v_new_le_id);

        INSERT INTO mp_movement_source_link (financial_movement_id, source_record_id, is_primary) VALUES (v_new_fm_id, v_new_sr_id, TRUE)
        RETURNING id INTO v_new_link_id;
        v_link_ids := array_append(v_link_ids, v_new_link_id);
...
GRANT EXECUTE ON FUNCTION import_financial_movements_reconciliation_v2(BIGINT, JSONB[], TEXT, DATE, DATE, TEXT) TO authenticated, service_role;
